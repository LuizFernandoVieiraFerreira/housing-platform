...
class AdminAuthorizationTest {
  ...
  void nonAdminIsForbidden(AdminRoute route) throws Exception {
    MockHttpServletRequestBuilder request =
        switch (route.method()) {
          case "get" -> get(route.path()).with(AuthorizationMvcTestSupport.bearerToken(customer));
          case "post" ->
              post(route.path()).with(AuthorizationMvcTestSupport.bearerToken(customer));
          case "patch" ->
              patch(route.path())
  ...
  @Test
  void adminServiceRejectsCustomerBeforeRepository() throws Exception {
    mockMvc
        .perform(
            get(AuthorizationMvcTestSupport.API + "/admin/stats")
                .with(AuthorizationMvcTestSupport.bearerToken(customer)))
        .andExpect(status().isForbidden())
        .andExpect(jsonPath("$.error.code").value("FORBIDDEN"));
  }
