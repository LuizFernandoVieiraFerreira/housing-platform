...
create table public.location_aliases (
  ...
  radius_meters integer not null default 2500,
  created_at timestamptz not null default timezone('utc', now()),
  constraint location_aliases_alias_not_blank check (char_length(trim(alias)) > 0),
  constraint location_aliases_district_not_blank check (char_length(trim(district)) > 0),
  constraint location_aliases_center_lat_range check (center_lat >= -90 and center_lat <= 90),
  constraint location_aliases_center_lng_range check (center_lng >= -180 and center_lng <= 180),
  constraint location_aliases_radius_positive check (radius_meters > 0)
);

create unique index location_aliases_alias_unique_idx on public.location_aliases (lower(trim(alias)));

create index location_aliases_district_idx on public.location_aliases (district);

alter table public.location_aliases enable row level security;

create policy "Location aliases are publicly readable"
on public.location_aliases
for select
to anon, authenticated
using (true);

create policy "Admins can manage location aliases"
on public.location_aliases
for all
to authenticated
...
as $$
  select *
  from public.search_properties_hybrid(p_filters, p_limit, p_offset);
$$;

grant execute on function public.search_properties_hybrid(jsonb, integer, integer) to anon, authenticated;
grant execute on function public.search_properties(jsonb, integer, integer) to anon, authenticated;

-- Amenity filtering in search reads property_amenities under RLS; anon needs table SELECT.
